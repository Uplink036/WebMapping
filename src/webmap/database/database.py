...
class Database:
    def __init__(self):
        if verify_connectivity() is False:
            raise ConnectionError("Database error: Could not create connection to database")
        self._driver = GraphDatabase.driver(DATABASE_URI, auth=(AUTH_USERNAME, AUTH_PASSWORD))
...
class Neo4JStack(StackDB):
    ...
    def pop(self) -> str:
        with self._driver.session() as session:
            result = session.run("MATCH (s:StackItem) WITH s ORDER BY elementId(s) DESC LIMIT 1 RETURN s.value, elementId(s) as node_id")
            ...
            session.run("MATCH (s:StackItem) WHERE elementId(s) = $node_id SET s:CompletedStackItem REMOVE s:StackItem", node_id=node_id)
            return value
...
class Neo4JGraph(GraphDB):
    ...
    def add_node(self, website: str) -> bool:
        with self._driver.session() as session:
            result = session.run("MERGE (w:Website {name: $name}) RETURN w", name=website)
    ...
    def add_edge(self, from_website: str, to_website: str) -> bool:
        with self._driver.session() as session:
            result = session.run("""
                ...
                MERGE (a)-[:LINKS_TO]->(b)
                RETURN a, b
            """, from_site=from_website, to_site=to_website)
            return result.single() is not None
    ...
    def in_database(self, website: str) -> bool:
        with self._driver.session() as session:
            result = session.run("MATCH (w:Website {name: $name}) RETURN w", name=website)
            return result.single() is not None